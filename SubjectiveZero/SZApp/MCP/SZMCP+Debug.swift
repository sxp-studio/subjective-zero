...
extension SZHostBridge {
    nonisolated static var debugToolDefinitions: [[String: Any]] {
        [
            ...
            tool("debug_chat_transcript", "Return a chat transcript as JSON — role/text/thinking plus, where present, timestamp/duration/usage/breakdown per message (the same numbers the in-app turn breakdown shows). A finished build carries `receipt` {label, outcome: end|stopped|declined|failed, detail?} and `graphRunID` — read those rather than parsing the text.",
