...
// update(). `amount` rides the scalar value channel (unconnected default 1.0, live-tunable).
...
final class Node: SZNode {
    ...
    func setup(_ ctx: SZSetupContext) {
        ...
        kernel void brightness(texture2d<float, access::read>  inTex  [[texture(0)]],
                               texture2d<float, access::write> outTex [[texture(1)]],
                               constant float &amount [[buffer(0)]],
                               uint2 gid [[thread_position_in_grid]]) {
            if (gid.x >= outTex.get_width() || gid.y >= outTex.get_height()) { return; }
            float4 c = inTex.read(gid);
            outTex.write(float4(c.rgb * amount, c.a), gid);
        }
        """
        guard let library = try? ctx.device.makeLibrary(source: source, options: nil),
              let function = library.makeFunction(name: "brightness"),
