...
// the host-derived `feed`; sending routes through `onSend` → host.sendChat (the same path as the
...
public struct SZChatPanel: View {
    ...
    // A node card's chat button: drop that node's mention at the caret and take focus.
    private let pendingMention: SZComposerMentionInjection?
    private let onConsumePendingMention: (UUID) -> Void
    ...
    public var body: some View {
        ...
        .onChange(of: pendingMention?.id) { applyPendingMention() }
        .onChange(of: scope) { applyPendingDraft() }
    }

    /// Insert the requested mention at the caret — the composer's own relay does it, so the token
    /// lands wherever you were typing rather than replacing what you had, and the field takes focus.
    private func applyPendingMention() {
        guard let injection = pendingMention else { return }
        onConsumePendingMention(injection.id)   // consumed either way — a stale nudge must not linger
        composerRelay.insertMention?(injection.candidate)
