...
// nobody has given a file yet. This tells those apart, and it is the same function behind the node's
// pill and behind `agent_check_path`, so the user and an agent never describe one fault in two
// vocabularies. The verdict that matters most is the last one: a file present and readable but the
// wrong kind, which a bare existence check calls "fine". What kinds a port takes comes from the port's
// own `ui.fileTypes` — nothing here knows any file format.
//
// A snapshot, not live state: one `stat` at the moment it is asked. The host re-asks on writes, on
// contract changes, on project open and when the app comes back to the front; between those a deleted
// file still reads clean. Never call it per frame or from a view body — a stalled network mount makes
// a `stat` block.
import Foundation
