...
public enum SZPortBindingAudit {
    ...
    public static func audit(contract: SZNodeContract, source: String, sourceFile: String = "Node.swift") -> Result {
        ...
        for name in referencedInputs.subtracting(declaredInputs).sorted() {
            errors.append("\(sourceFile) reads input port \"\(name)\" but node-contract.json declares no such input.")
        }
        ...
        if !scan.contains("func setPaused(") {
            for type in liveResourceTypes where scan.contains("\(type)(") {
                errors.append("""
                    \(sourceFile) creates an \(type), which keeps running when the graph's clock stops \
        ...
        for name in declaredInputs.subtracting(referencedInputs).sorted() {
            warnings.append("input \"\(name)\" is declared in node-contract.json but never read in \(sourceFile) (dead control?).")
        }
        for name in declaredOutputs.subtracting(referencedOutputs).sorted() {
            warnings.append("output \"\(name)\" is declared in node-contract.json but never written in \(sourceFile).")
        }
    ...
    public static func auditForPromote(source: String, authored: SZNodeContract?, node: SZNode,
        ...
        let result = audited.map { audit(contract: $0, source: source, sourceFile: sourceFile) } ?? Result(errors: [], warnings: [])
    ...
    private static func channelErrors(contract: SZNodeContract, scan: String, sourceFile: String) -> [String] {
        var errors: [String] = []
        for direction in [Direction.input, .output] {
            ...
            for (channel, accessors) in accessorChannels[direction] ?? [] {
                for name in portNames(in: scan, accessors: accessors).sorted() {
                    ...
                    errors.append("""
                        \(sourceFile) \(verb) \(direction.rawValue) port "\(name)" as \(channel.reading), but \
    ...
    /// `inputFloats(` / `inputFloatArray(`). Single quotes count too: a web node's `Node.js` writes
    /// `ctx.inputFloat('amount')`.
