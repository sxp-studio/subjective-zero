...
// requests the build — the run is the reply — `amend` folds words into work already scheduled,
// and everything else is answered in conversation, cold or resumed.
...
let step = SZStep(outcomes: ["build", "answer", "answer-resumed", "implement", "amend"]) { ctx in
    ...
    // With nothing scheduled there is nothing to fold into, so the ruling cannot stand: the ask
    // becomes a fresh one. The check is here rather than in the prompt because a hallucinated
    // amend would otherwise route to a turn with no work to do.
