...
// folds words into work already scheduled or already building, and everything else is answered in
// conversation, cold or resumed.
...
let step = SZStep(outcomes: ["build", "convert", "answer", "answer-resumed", "implement", "amend"]) { ctx in
    if ctx.run?.intent == "convert" { return "convert" }   // SZRunIntent.convert; the facts carry its raw value
