...
final class Node: SZNode {
    ...
    func update(_ ctx: SZFrameContext) {
        ...
        guard running, let pipeline, let cache = textureCache, let pixelBuffer = delegate?.latest(),
              let camera = Self.texture(from: pixelBuffer, cache: cache) else { return }  // black until a frame arrives
    ...
    private static func texture(from pixelBuffer: CVPixelBuffer, cache: CVMetalTextureCache) -> (any MTLTexture)? {
