...
- USER connected MacBook Camera.output → Grayscale Effect.input
- DIRECTOR re-prompted Grayscale Effect
- Coding Agent (Grayscale Effect) toggled display → Grayscale Effect.output
...
- The blocker is a port-audit line (`Node.swift reads input port "x" but node-contract.json declares no
  ...
  "make the files match verbatim" theories, and do not re-brief a BUILT node (`"kind": "generated"`) that
  `agent_read_node` reports with no `rebuildReason` at all: there is nothing flagged for an agent to fix.
  That reads on BUILT nodes only — a never-built draft (`"kind": "prompt"`) never carries a
